...
namespace WhatIsClass
{
    ...
    // 이름, 나이, 주소
        ...
        {
            // 구조체 BusinessCard를 가져와서 businessCard로 선언해서 사용한다.
            BusinessCard businessCard;

            businessCard.myName = "Lim Junil";
            businessCard.myOld = 25;
            businessCard.myHome = "가평";

            Console.WriteLine("제 이름은 {0},\n나이는 {1},\n살고 있는 주소는 {2} 입니다.",
                businessCard.myName, businessCard.myOld, businessCard.myHome);
        }


        enum Weekday
        {
            SUNDAY, MONDAT, TUESDAY, WEDNESDAY, THURSDAY, FRIDAY, SATURDAY
        }
        ...
        static void WhatIsStructure()
        {
             ...
             * 앞에서 배운 변수는 이름 하나로 공간을 하나 갖고, 배열은 이름 하나로 데이터 형식이 동일한
             * 공간을 여러 개 갖는다.
             ...
             * 여러 항목의 모임이다.
             * 즉, 구조체 변수란 이름 하나로 데이터 형식 1개 이상을 하나로 보관해 놓는 그릇 역할을 하는
             * 것이다.
             * 그리고 구조체 배열은 이름 하나로 데이터 형식 1개 이상을 여러 개 보관해놓는 그릇 역할을 한다.
             * 
             ...
             * 구조체 만들기
             * 구조체를 만드는 방법은 다음과 같다.
             * 구조체를 의미하는 struct 키워드를 사용하여 구조체를 만들고 중괄호 안에 구조체 맴버들을 생성한다.
             * ex) 
             *   struct [구조체 이름]
             *   {
             *       [데이터 형식] [변수 이름 1];
             ...
             *   
             *   
             * 여기에서 구조체 이름은 새로운 데이터 형식이 되며, 변수를 선언할 때 구조체 이름을
             * 사용할 수 있다.
            ...
            point.x = 100;
            point.y = 200;
            point.myNameJunIl = "JunIl";

            Console.WriteLine("x, y 좌표는 ({0}, {1}) 입니다.", point.x, point.y);

            /**
             * 구조체를 선언할 때는 struct 키워드를 사용한다.
             * 여기에서는 struct Point {} 형태로 이름이 Point인 구조체를 만들었다.
             * 만든 구조체는 int, string 형식의 변수 선언과 동일한 방법으로 선언한 후 사용할 수 있다.
             * 변수는 값을 하나만 저장하지만, 구조체는 점(.)으로 구분하여 구조체를 선언할 때 사용한
             * 변수 여러 개를 이름 하나(point)로 묶어 쓸 수 있다.
             * 
             * 
             */
             ...
             * 관리하는 표현 방식이다. 일반적으로 열거형을 줄여 말한다.
             * 열거형을 사용하면 값 리스트 여러 개를 이름 하나로 관리할 수 있는 장점이 있다.
            ...
            //Console.WriteLine("red 아하 여기가 색이 바뀌구나");
            //Console.WriteLine("red 아하 여기가 색이 바뀌구나");
            //Console.WriteLine("red 아하 여기가 색이 바뀌구나");
            //Console.WriteLine("red 아하 여기가 색이 바뀌구나");
            //Console.ResetColor();
