...
namespace WhatIsInterface
{
    internal class Description
    {
         ...
         * (중요!)이 강의에서는 프로그램의 표준 규약을 정하고 따를 수 있도록 강제하는 인터페이스(Interface)를 학습해
         * 보겠다.
         * 인터페이스는 클래스에서 구현해야 하는 관련 기능의 정의가 포함된 개념이다.
         * 
         * 
         * 인터페이스
         * (지금까지 배운 UI 와 같은 의미가 아님, 중요)
         * 
         * 인터페이스는 클래스 또는 구조체에 포함될 수 있는 관련있는 메서드들을 묶어 관리한다.
         * 인터페이스는 명세서(규약, 표준) 역할을 한다.
         * (중요!)인터페이스를 상속받아 그 내용을 구현하는 클래스는 인터페이스에 선언된 멤버(속성, 메서드 등)가
         * 반드시 구현되어 있다는 보증을 한다.
         * 
         ...
         * - (중요!)단일 상속만 지원하는 클래스와 달리 인터페이스를 사용한 다중 상속이 가능하다.
         ...
         * - 인터페이스는 인스턴스화 되지 않는다. 클래스를 사용해서 인스턴스화한다.
         * - 인터페이스는 계약(Contract) 의미가 강하며 속성, 메서드, 이벤트, 인덱서 등 구조를 미리 정의한다.
         * 
         * 추상적 : 내용이 정해져 있지 않아서 생각할 요소가 있어 추상적이다.
         */
         ...
         * c#에서 클래스는 클래스에 대한 단일 상속만 지원하는 대신,
         ...
         * 
         * 추상 클래스
         * - 구현된 코드가 들어온다. 즉, 메서드 시그니처만 있는 것이 아니라 사용 가능한 실제로 구현된 메서드도
         *   들어온다.
