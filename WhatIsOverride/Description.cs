...
namespace WhatIsOverride
{
    public class Description
    {
        public void OverrideDesc()
        {
             ...
             * 메서드 오버라이드
             * 
             * 부모 클래스에 만든 메서드를 자식 클래스에서 다시 새롭게 만들어 사용하는 것을
             * 메서드 오버라이드라고 한다.
             * 
             * 메서드 오버라이드 : 재정의
             * 
             * 클래스 관계를 따지는 상속 개념에서 부모 클래스에 이미 만든 메서드를 동일한 이름으로
             * 자식 클래스에서 다시 정의(재정의) 해서 사용한다는 개념이 메서드 오버라이드라고 한다.
             * 
             * 
             ...
             * 
             * ex) 오버로드는 부모 클래스나 한 클래스 내에서 인자를 다르게 하거나 해서 같은 함수를 여러 번 사용하는 것,
             * 오버라이드는 virtual을 선언한 부모 클래스의 함수를 자식 클래스에서 override 선언해서 같은 이름의 함수를
             * 가져와 재선언해서 사용한다. 예를 들어 부모 클래스에서 int i = 0; 이라 선언했으면 자식 클래스의 오버라이드
             * 함수에서 i = 10; 으로 재정의 할 수 있다는 것이다.
             * 
             * 
             * 
             * 가상 메서드
    ...
    public class Parent
    {
        ...
        // 한 클래스 내에만 있고, 인자를 다르게 해서 여러 개 만드는게 메서드 오버로드
    ...
    public class Child : Parent
    {
        ...
        {
            // 내 부모 클래스 껄 쓰겠다. 라는 의미
            base.Run();
            // 정의 안하고 쓰면 부모의 number 값을 그대로 씀
            Console.WriteLine("number : {0}", number);

            // 오버라이드 한 상태에서 number을 재정의하면 값이 바뀜
            // 꼭 this. 붙여서 헷갈리지 않게 하자.
            this.number = 10;
            Console.WriteLine("number : {0}", this.number);

        }

        // 오버라이드는 재정의, 부모 클래스에 선언된 내용을 재정의해서 사용한다.
    ...
    public class Button
    {
        ...
        {
            // 이 버튼을 누르면 퀘스트 창이 열림. <- 상속할거면 기능을 정해두면 좋지 않음
